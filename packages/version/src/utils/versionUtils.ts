 ...
 */
export function generateBuildId(timestamp: number): string {
 ...
 */
export async function fetchServerVersion(endpoint: string = DEFAULT_CONFIG.versionEndpoint): Promise<VersionInfo | null> {
  try {
 ...
 */
export function hasNewVersion(current: VersionInfo | null, server: VersionInfo): boolean {
 ...
 */
export function shouldEnableVersionCheck(config: VersionCheckConfig = {}): boolean {
 ...
 */
export function isVersionDismissed(buildId: string): boolean {
  if (typeof window === 'undefined') return false;
 ...
 */
export function dismissVersion(buildId: string): void {
  if (typeof window === 'undefined') return;
 ...
 */
export function initializeVersion(): VersionInfo | null {
  if (typeof window === 'undefined') return null;
