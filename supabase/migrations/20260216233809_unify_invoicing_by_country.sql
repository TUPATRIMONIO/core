-- =====================================================
-- Migration: Unify invoicing by country
-- Description: Unifica la lógica de facturación para que todas las organizaciones de Chile
--              (personales y empresas) usen Haulmer (Boleta/Factura), y las internacionales usen Stripe Invoice.
-- Created: 2026-02-16
-- =====================================================
...
BEGIN
  ...
  -- Obtener proveedor del pago asociado a la orden (solo para referencia/logging)
  SELECT provider INTO v_payment_provider
  FROM billing.payments
  WHERE id = NEW.payment_id;
  
  -- =================================================================================
  -- NUEVA LÓGICA: Determinar tipo de documento según PAÍS de la organización
  -- =================================================================================
  -- Chile (CL) -> Haulmer (Boleta o Factura)
  -- Resto del mundo -> Stripe Invoice
  
  IF v_org_country = 'CL' THEN
    -- Para Chile, usar el tipo especificado en metadata (boleta/factura)
    -- Si no viene especificado, por defecto es boleta_electronica
    v_document_type := COALESCE(
      (v_order_metadata->>'document_type')::invoicing.document_type,
      'boleta_electronica'::invoicing.document_type
    );
  ELSE
    -- Para el resto del mundo, siempre Stripe Invoice
    v_document_type := 'stripe_invoice'::invoicing.document_type;
  END IF;
  
  RAISE NOTICE '[invoicing] Tipo de documento determinado: % (país: %, proveedor: %, metadata: %)', 
    v_document_type, v_org_country, v_payment_provider, v_order_metadata;
...
DO $$ 
BEGIN 
  RAISE NOTICE '✅ Trigger on_order_completed actualizado: Lógica unificada por país (CL=Haulmer, Otros=Stripe)';
END $$;
