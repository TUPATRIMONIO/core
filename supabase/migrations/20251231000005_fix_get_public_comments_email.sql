-- =====================================================
-- Migration: Fix get public document comments RPC (email column)
-- Description: Fix reference to non-existent email column in core.users
-- Created: 2025-12-31
-- =====================================================

-- Create RPC to get comments for a public document
-- Uses auth.users for email since core.users doesn't have email column
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, documents, core, auth
AS $$
DECLARE
...
BEGIN
  ...
  -- Using auth.users for email since core.users doesn't have email
    ...
    jsonb_build_object(
      ...
      'author_name', CASE 
        WHEN c.user_id IS NOT NULL THEN 
          COALESCE(
            (SELECT CONCAT_WS(' ', u.first_name, u.last_name) FROM core.users u WHERE u.id = c.user_id),
            'Usuario'
          )
      ...
      'author_email', CASE 
        WHEN c.user_id IS NOT NULL THEN 
          (SELECT au.email FROM auth.users au WHERE au.id = c.user_id)
        WHEN c.visitor_access_id IS NOT NULL THEN 
          pal.visitor_email
        ELSE NULL
      END,
...
DO $$ 
BEGIN 
  RAISE NOTICE '✅ RPC get_public_document_comments fixed (email column)';
END $$;
