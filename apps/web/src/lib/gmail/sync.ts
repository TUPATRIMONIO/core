...
import { getSharedGmailAccount, getOAuth2Client } from './client'
...
export function parseEmailHeaders(message: any): {
  ...
  const date = dateStr ? new Date(dateStr) : new Date(message.internalDate ? parseInt(message.internalDate) : Date.now())
  
  // Extraer Message-ID (crucial para threading)
  const messageIdHeader = getHeader('Message-ID')
  // Normalizar: remover <> si existen
  const messageId = messageIdHeader ? messageIdHeader.replace(/^<|>$/g, '').trim() : undefined
  
  const inReplyTo = getHeader('In-Reply-To')
  // Normalizar In-Reply-To: remover <> si existen
  const normalizedInReplyTo = inReplyTo ? inReplyTo.replace(/^<|>$/g, '').trim() : undefined
...
export function parseGmailMessage(message: any): ParsedEmail {
  ...
  const hasAttachments = message.payload?.parts?.some((p: any) => 
...
export async function getUnreadEmails(
  ...
  if (!account || !account.tokens) {
    throw new Error('No hay cuenta Gmail compartida configurada o tokens inválidos')
  }
  ...
  if (messages.length === 0 && !includeRead) {
    console.log('⚠️ No se encontraron mensajes no leídos. Intentando buscar todos los emails recientes...')
...
export async function getTicketForThread(
  ...
  const { data: ticketId, error } = await supabase.rpc('get_ticket_for_gmail_thread', {
    p_organization_id: organizationId,
    p_gmail_thread_id: gmailThreadId
  })

  if (error) {
    console.error('⚠️ Error en get_ticket_for_gmail_thread RPC:', error.message)
    ...
    if (countError) {
      console.error('❌ Error buscando emails con thread_id:', countError.message)
    } else {
      console.log(`🔍 Emails con thread_id ${gmailThreadId}: ${allEmailsWithThread?.length || 0}`)
      if (allEmailsWithThread && allEmailsWithThread.length > 0) {
        allEmailsWithThread.forEach(e => {
          console.log(`   - Email ${e.id}: direction=${e.direction}, ticket_id=${e.ticket_id || 'null'}, subject="${e.subject}"`)
        })
      }
    }
  ...
  if (ticketId) {
    ...
    console.log(`📭 RPC no encontró ticket para thread_id: ${gmailThreadId}`)
  }
...
export async function findExistingTicketBySubjectAndContact(
  ...
  // Intentar usar RPC function
  const { data: ticketId, error } = await supabase.rpc('get_ticket_for_contact_subject', {
    p_organization_id: organizationId,
  ...
  if (error) {
    console.error('⚠️ Error en get_ticket_for_contact_subject RPC:', error.message)
    ...
    const cleanSubject = subject.replace(/^(Re:|Fwd:|RE:|FW:|re:|fwd:)\s*/gi, '').trim()
    ...
    for (const t of ticket || []) {
      const ticketCleanSubject = t.subject.replace(/^(Re:|Fwd:|RE:|FW:|re:|fwd:)\s*/gi, '').trim()
      if (
        ...
        console.log(`✅ Fallback encontró ticket por subject: ${t.id} (subject: "${t.subject}")`)
        return t.id
      }
    }

    console.log(`📭 No se encontró ticket con subject similar para contacto ${contactEmail}`)
    return null
  }
  ...
  if (ticketId) {
    ...
    console.log(`📭 RPC no encontró ticket para subject: "${subject}" y contacto: ${contactEmail}`)
  }
...
export async function findOrCreateContact(
  ...
  if (error || !newContact) {
    throw new Error(`Error creando contacto: ${error?.message || 'Unknown error'}`)
  }
