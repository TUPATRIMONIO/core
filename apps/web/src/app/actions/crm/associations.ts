        ...
        switch (type) {
            case "contact":
                if (
                    ...
                    const { error: userLinkError } = await supabase
                        .schema("crm")
                        .from("ticket_users")
                        .insert({ ticket_id: ticketId, user_id: entityId });
                    error = userLinkError;
                } else {
                    ...
                    const { error: contactLinkError } = await supabase
                        ...
                        .insert({ ticket_id: ticketId, contact_id: entityId });
            ...
            case "company":
                if (
                    ...
                    const { error: companyLinkError } = await supabase
                        .schema("crm")
                        .from("ticket_companies")
                        .insert({ ticket_id: ticketId, company_id: entityId });
            ...
            case "order":
                const { error: orderErr } = await supabase
                    .schema("crm")
                    .from("ticket_orders")
                    .insert({ ticket_id: ticketId, order_id: entityId });
                error = orderErr;
                break;
        }
        ...
        switch (type) {
            case "contact":
                if (isPlatform) {
                    ...
                    const { error: userErr } = await supabase
                        ...
                        .from("ticket_users")
                        .delete()
                        .match({ ticket_id: ticketId, user_id: entityId });
                    error = userErr;
                } else {
                    ...
                    const { error: contactErr } = await supabase
                        ...
                        .from("ticket_contacts")
                        .delete()
                        .match({ ticket_id: ticketId, contact_id: entityId });
                    error = contactErr;
                }
                break;
            ...
            case "company":
                if (isPlatform) {
                    ...
                    const { error: companyErr } = await supabase
                        ...
                        .from("ticket_companies")
                        .delete()
                        .match({ ticket_id: ticketId, company_id: entityId });
                    error = companyErr;
                }
                break;
            ...
            case "order":
                const { error: orderErr } = await supabase
                    ...
                    .from("ticket_orders")
                    .delete()
                    .match({ ticket_id: ticketId, order_id: entityId });
                error = orderErr;
                break;
        }
