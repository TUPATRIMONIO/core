...
import { getUnreadEmails, getTicketForThread, findOrCreateContact, findExistingTicketBySubjectAndContact, ParsedEmail } from '@/lib/gmail/sync'
    ...
    for (const email of unreadEmails) {
      try {
        ...
        if (existingEmail) {
          ...
          console.log(`[CRON] ⏩ Email ${email.messageId} ya existe en BD${existingEmail.ticket_id ? `, asociado a ticket ${existingEmail.ticket_id}` : ''}. Saltando.`)
          results.processed++
          continue
        }

        // PASO 2: Verificar si este thread ya tiene ticket
        console.log(`[CRON] 🔍 Buscando ticket para thread_id: ${email.threadId} (email: ${email.subject})`)
        const existingTicketIdByThread = await getTicketForThread(organizationId, email.threadId)
        console.log(`[CRON] 🔍 Resultado getTicketForThread: ${existingTicketIdByThread || 'null'}`)
        ...
        if (existingTicketIdByThread) {
          ...
          if (ticketCheck) {
            console.log(`[CRON] 📧 Email ${email.messageId} pertenece a thread existente, agregando a ticket ${existingTicketIdByThread}`)
            const result = await updateTicketWithEmail(serviceSupabase, existingTicketIdByThread, email, organizationId, userId)
            if (result === 'created') {
        ...
        const existingTicketIdBySubject = await findExistingTicketBySubjectAndContact(
          organizationId,
          email.subject,
          email.fromEmail
        )
        ...
        if (existingTicketIdBySubject) {
          console.log(`[CRON] 📧 Email ${email.messageId} tiene subject similar a ticket existente ${existingTicketIdBySubject}, agregando al ticket`)
          const result = await updateTicketWithEmail(serviceSupabase, existingTicketIdBySubject, email, organizationId, userId)
          if (result === 'created') {
            results.ticketsCreated++
          } else {
            results.ticketsUpdated++
        ...
        console.log(`[CRON] 🆕 Creando nuevo ticket para email ${email.messageId} (subject: ${email.subject})`)
        await createTicketFromEmail(serviceSupabase, email, organizationId, userId)
        results.ticketsCreated++
        results.processed++
          console.log(`⏭️ [CRON] Email ${email.messageId} ya procesado, asociado a ticket ${existingEmail.ticket_id}`)
          results.processed++
          continue
        }

        // SEGUNDO: Verificar si este thread ya tiene ticket
        const existingTicketId = await getTicketForThread(organizationId, email.threadId)

        if (existingTicketId) {
          // Actualizar ticket existente (o crear si no existe)
          const result = await updateTicketWithEmail(serviceSupabase, existingTicketId, email, organizationId, userId)
          if (result === 'created') {
            results.ticketsCreated++
          } else {
            results.ticketsUpdated++
        ...
        console.error(`❌ [CRON] Error procesando email ${email.messageId}:`, error)
        results.errors.push(`Email ${email.messageId}: ${error.message}`)
      }
    }

    console.log(`✅ [CRON] Sincronización completada: ${results.processed} procesados, ${results.ticketsCreated} creados, ${results.ticketsUpdated} actualizados`)

    return NextResponse.json({
      success: true,
      results,
      message: `Procesados ${results.processed} emails. ${results.ticketsCreated} tickets creados, ${results.ticketsUpdated} actualizados.`,
    })
  } catch (error: any) {
...
async function createTicketFromEmail(
  ...
  if (thread) {
    ...
    if (threadError || !newThread) {
      throw new Error(`Error creando thread: ${threadError?.message || 'Unknown error'}`)
    }
  ...
  if (ticketError || !ticket) {
    throw new Error(`Error creando ticket: ${ticketError?.message || 'Unknown error'}`)
  }
...
async function updateTicketWithEmail(
  ...
  if (existingEmail) {
    console.log(`⏭️ [CRON] Email ${email.messageId} ya existe en BD, saltando inserción`)
  } else {
    // Obtener cuenta Gmail compartida para received_in_account_id
    const { data: gmailAccount } = await supabase
      .from('email_accounts')
