...
export async function POST(req: Request) {
  try {
    ...
    if (docError || !doc?.id) {
      console.error('[api/signing/analyze] Documento no encontrado:', { documentId, docError })
    ...
    if (!serviceRoleKey) {
      console.error('[api/signing/analyze] SUPABASE_SERVICE_ROLE_KEY no configurada')
      return NextResponse.json({ error: 'Configuración incompleta del servidor' }, { status: 500 })
    }

    if (!supabaseUrl) {
      console.error('[api/signing/analyze] NEXT_PUBLIC_SUPABASE_URL no configurada')
      return NextResponse.json({ error: 'Configuración incompleta del servidor' }, { status: 500 })
    }
    ...
    const reviewId = ((json as { review_id?: string; reviewId?: string })?.review_id
      || (json as { review_id?: string; reviewId?: string })?.reviewId
      || '').toString()
    const statusFromJson = ((json as { status?: string })?.status || '').toString()
    ...
    if (reviewId) {
      try {
        ...
        const { data: review, error: reviewError } = await supabaseAdmin
          ...
          .select('id, status, reasons, suggestions, metadata, completed_at, created_at, confidence_score, passed, ai_model, prompt_used')
          .eq('id', reviewId)
          .maybeSingle()

        if (reviewError) {
          console.warn('[api/signing/analyze] No se pudo obtener ai_review por reviewId:', reviewError.message)
        } else {
          aiReviewData = review
        }
      } catch (adminError) {
        console.warn('[api/signing/analyze] Error consultando ai_review con service role:', adminError)
      }
    }
    ...
    if (!aiReviewData) {
      const reasons = Array.isArray((json as { reasons?: unknown[] })?.reasons)
        ...
        : null
      const suggestions = Array.isArray((json as { suggestions?: unknown[] })?.suggestions)
        ? (json as { suggestions?: unknown[] })?.suggestions
        : null
      const summary = (json as { summary?: string })?.summary
      const confidenceScore = (json as { confidence_score?: number })?.confidence_score
      const passedValue = (json as { passed?: boolean })?.passed

      if (reasons || suggestions || summary) {
        aiReviewData = {
          id: reviewId || null,
          status: statusFromJson || (passedValue === true ? 'approved' : 'rejected'),
          reasons: reasons || [],
