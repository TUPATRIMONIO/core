...
export async function GET(request: NextRequest) {
  try {
    ...
    // Verificar si hay filtros que obliguen a procesar en memoria (email content)
    const hasEmailFilters = dateFrom || dateTo || fromEmail || toEmail ||
      subject || bodyText || unreadOnly;

    // Si hay filtros complejos de email, la paginación de BD se aplica DESPUÉS del filtrado
    // Si NO hay filtros complejos, aplicamos paginación directa a la query inicial
    ...
    // Si NO hay filtros de email, paginar directamente en la DB
    if (!hasEmailFilters) {
    ...
    if (hasEmailFilters) {
      ...
      if (
        subject && !dateFrom && !dateTo && !fromEmail && !toEmail &&
        !bodyText && !unreadOnly
      ) {
        ...
        if (bodyText) {
          ...
          emailQuery = emailQuery.or(
            `body_text.ilike.%${bodyText}%,body_html.ilike.%${bodyText}%`,
          );
        }
        if (unreadOnly) {
          emailQuery = emailQuery.eq("is_read", false).eq(
            "direction",
            "inbound",
          );
        }
        ...
        const matchingTicketIds = new Set(
          filteredEmails?.map((e: any) => e.ticket_id) || [],
        );
        ...
        filteredTickets = filteredTickets.filter((t: any) =>
          matchingTicketIds.has(t.id)
        );
      }

      // Actualizar total count después de filtrar en memoria
      totalCount = filteredTickets.length;

      // Aplicar paginación manual a los resultados filtrados
      // NOTA: Esto no es ideal para grandes volúmenes de datos con filtros complejos,
      // pero mantiene la consistencia funcional por ahora.
      const startIndex = (page - 1) * limit;
      filteredTickets = filteredTickets.slice(startIndex, startIndex + limit);
    }
