...
/**
 * API Route: Responder a un ticket (envía email y guarda)
 * 
...
export async function POST(
  ...
  try {
    ...
    if (!isPlatformAdmin) {
      return NextResponse.json(
        { error: 'Solo los platform admins pueden responder tickets' },
        { status: 403 }
      )
    }
    ...
    console.error('Error en POST /api/admin/tickets/[id]/reply:', error)
    return NextResponse.json(
