...
export async function POST(request: NextRequest) {
  try {
    ...
    if (unreadEmails.length === 0) {
      console.log('⚠️ No hay emails no leídos. Intentando con todos los emails del inbox...')
      unreadEmails = await getUnreadEmails(organizationId, 20, true)
      console.log(`📧 Total de emails obtenidos (incluyendo leídos): ${unreadEmails.length}`)
    }
    ...
    for (const email of unreadEmails) {
      try {
        ...
        const { data: existingEmails, error: existingEmailError } = await serviceSupabase
        ...
        if (existingEmailError) {
          console.log('⚠️ RPC check_email_exists no existe, usando query directa')
        ...
        console.log(`🔍 Buscando ticket para thread_id: ${email.threadId} (email: ${email.subject})`)
        const existingTicketIdByThread = await getTicketForThread(organizationId, email.threadId)
        console.log(`🔍 Resultado getTicketForThread: ${existingTicketIdByThread || 'null'}`)
        ...
        if (existingTicketIdByThread) {
          ...
          if (ticketCheck) {
            console.log(`📧 Email ${email.messageId} pertenece a thread existente, agregando a ticket ${existingTicketIdByThread}`)
            const result = await updateTicketWithEmail(serviceSupabase, existingTicketIdByThread, email, organizationId, user.id)
            if (result === 'created') {
            ...
            console.log(`⚠️ Ticket ${existingTicketIdByThread} no existe, continuando con otras verificaciones...`)
          }
        }

        // PASO 3: Verificar si hay un ticket existente con el mismo subject del mismo contacto
        console.log(`🔍 Buscando ticket por subject: "${email.subject}" y contacto: ${email.fromEmail}`)
        const existingTicketIdBySubject = await findExistingTicketBySubjectAndContact(
          organizationId,
          email.subject,
          email.fromEmail
        )
        console.log(`🔍 Resultado findExistingTicketBySubjectAndContact: ${existingTicketIdBySubject || 'null'}`)

        if (existingTicketIdBySubject) {
          console.log(`📧 Email ${email.messageId} tiene subject similar a ticket existente ${existingTicketIdBySubject}, agregando al ticket`)
          const result = await updateTicketWithEmail(serviceSupabase, existingTicketIdBySubject, email, organizationId, user.id)
          if (result === 'created') {
            results.ticketsCreated++
          } else {
            results.ticketsUpdated++
        ...
        console.log(`🆕 Creando nuevo ticket para email ${email.messageId} (subject: ${email.subject})`)
        await createTicketFromEmail(serviceSupabase, email, organizationId, user.id)
        results.ticketsCreated++
        results.processed++
      } catch (error: any) {
...
async function updateTicketWithEmail(
  ...
  if (existingEmail) {
    console.log(`⏭️ Email ${email.messageId} ya existe en BD, saltando inserción`)
  } else {
    // Obtener cuenta Gmail compartida para received_in_account_id
    const { data: gmailAccount } = await supabase
      .from('email_accounts')
