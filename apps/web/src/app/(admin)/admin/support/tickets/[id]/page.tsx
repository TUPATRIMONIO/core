...
import { SupportTicketAdminReplyForm } from "@/components/admin/SupportTicketAdminReplyForm";
import { getAdminSupportTicketById } from "@/app/actions/support-tickets";
...
export default async function AdminSupportTicketDetailPage({ params }: PageProps) {
  ...
  const { ticket, messages } = await getAdminSupportTicketById(id);
      ...
      <div className="space-y-6">
        ...
        <div>
          <Button asChild variant="outline">
            <Link href="/admin/support/tickets">Volver al listado</Link>
          </Button>
        </div>
      </div>
    );
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between text-xs text-muted-foreground mb-2">
                        <span>{isUser ? "Usuario" : "Equipo"}</span>
                        <span>
