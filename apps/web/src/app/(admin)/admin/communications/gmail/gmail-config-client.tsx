  ...
  useEffect(() => {
    ...
    if (success || error) {
      const timer = setTimeout(() => {
        router.replace('/admin/communications/gmail')
      }, 5000)
      ...
      {error && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            ...
            {error === 'forbidden' && 'No tienes permisos para conectar cuentas Gmail'}
            {error === 'update_failed' && 'Error al actualizar la cuenta existente'}
            {error === 'create_failed' && 'Error al crear la cuenta'}
            {!['missing_params', 'invalid_state', 'unauthorized', 'forbidden', 'update_failed', 'create_failed'].includes(error) && 
              `Error: ${error}`}
          </AlertDescription>
        </Alert>
      )}
      ...
      {gmailAccount && (
        <Card>
          ...
          <CardContent className="space-y-2">
            <Link href="/admin/communications/gmail/signature">
              <Button variant="outline" className="w-full justify-start">
                <Mail className="mr-2 h-4 w-4" />
                Configurar mi firma personal
              </Button>
            </Link>
            <Link href="/admin/communications/test-email">
              <Button variant="outline" className="w-full justify-start">
                <Mail className="mr-2 h-4 w-4" />
